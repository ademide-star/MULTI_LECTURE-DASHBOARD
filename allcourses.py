...
# ===============================================================
# 🔧 MISSING HELPER FUNCTIONS FOR ADMIN VIEW
# ===============================================================
...
def get_courses():
    ...
    try:
        ...
        return list(courses_config.keys())
    except:
        return ["BCH201 - Biochemistry", "CSC201 - Computer Science"]

def load_courses_config():
    """Load courses configuration from JSON file"""
    try:
        config_file = "data/courses.json"
        if os.path.exists(config_file):
            ...
            return {
                "BCH201 - Biochemistry": "BCH201",
                "CSC201 - Computer Science": "CSC201"
            }
    except:
        return {
            "BCH201 - Biochemistry": "BCH201",
...
def get_seminar_submissions(course_code):
    """Get all seminar submissions for a course"""
    try:
        submissions_file = f"data/{course_code}/seminar_submissions.json"
...
def get_seminar_feedback(student_matric, course_code):
    """Get seminar feedback for a student"""
    try:
        feedback_file = f"data/{course_code}/seminar_feedback/{student_matric}_feedback.json"
        if os.path.exists(feedback_file):
            with open(feedback_file, 'r') as f:
                feedback_data = json.load(f)
                return feedback_data.get('feedback_text', 'No feedback available')
        return None
    except:
        return None

def save_seminar_feedback(student_matric, course_code, feedback_file, feedback_text):
    """Save seminar feedback for a student"""
    try:
        feedback_dir = f"data/{course_code}/seminar_feedback"
        ...
        feedback_json_file = os.path.join(feedback_dir, f"{student_matric}_feedback.json")
        with open(feedback_json_file, 'w') as f:
            json.dump(feedback_data, f, indent=2)
        
        # Save feedback file if provided
        if feedback_file:
            file_extension = os.path.splitext(feedback_file.name)[1]
            feedback_pdf_file = os.path.join(feedback_dir, f"{student_matric}_feedback{file_extension}")
            with open(feedback_pdf_file, 'wb') as f:
                f.write(feedback_file.getbuffer())
...
# ===============================================================
# 🔧 OTHER MISSING FUNCTIONS THAT MIGHT BE NEEDED
# ===============================================================

def verify_admin_password(course_code, password):
    """Verify admin password for a course"""
    try:
        stored_password = get_course_password(course_code)
...
def get_course_password(course_code):
    """Get course password"""
    try:
        password_file = f"data/{course_code}/password.txt"
...
def ensure_directories():
    ...
    try:
        ...
        for course in get_courses():
            ...
            os.makedirs(f"data/{course_code}", exist_ok=True)
            os.makedirs(f"data/{course_code}/announcements/pdfs", exist_ok=True)
            os.makedirs(f"data/{course_code}/seminar_feedback", exist_ok=True)
    except:
        pass

def get_persistent_path(file_type, course_code, filename=""):
    """Get path for persistent file storage"""
    base_dir = f"data/{course_code}/{file_type}"
    os.makedirs(base_dir, exist_ok=True)
    return os.path.join(base_dir, filename) if filename else base_dir

def get_file(course_code, file_type):
    """Get file path for course data"""
    return f"data/{course_code}/{file_type}.csv"                    
